    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            List<Song> songsStartsA = Songs
                                           .Where(s => s.Name.ToLower().StartsWith('a'))
                                           .ToList();
            Console.WriteLine("Songs that start with the letter 'a' : {0} songs", songsStartsA.Count());
            songsStartsA.ForEach(so => Console.WriteLine(so.Name));

            // - how many artists end with letter 'a' (case insensitive)
            
            List<Artist> artistsStartsA = Artists
                                                .Where(a => a.FullName.ToLower().EndsWith('a'))
                                                .ToList();
            Console.WriteLine("\nArtists that end with letter 'a' : {0} artists", artistsStartsA.Count());
            artistsStartsA.ForEach(ar => Console.WriteLine(ar.FullName));

            // - whats the name of the song with longest duration
            Song songLongestDuration = Songs
                                            .Where(sl => sl.Duration.Equals(Songs.Max(y => y.Duration)))
                                            .FirstOrDefault();
            Console.WriteLine("\nThe name of the song with longest duration is : {0}", songLongestDuration.Name);

            // - whats the total Duration of all Songs
            int totalDuration = Songs
                                    .Select(td => td.Duration)
                                    .Sum();
            Console.WriteLine("\nThe total Duration of all Songs is: {0} seconds", totalDuration);

            // - how many albums have Songs longer than 300 seconds
            int albumSongsLonger300 = Albums
                                            .Where(sl => sl.Songs.Any(x => x.Duration > 300))
                                            .Count();
            Console.WriteLine("\nThere are {0} albums that have Songs longer than 300 seconds.", albumSongsLonger300);

            // - print the names of the artists(separated with "--"), that have more than one album of PopRock genre
            List<string> nameArtistPopRock = Artists
                                          .Where(na => na.Albums.Any(x => x.Genre == Genre.PopRock))
                                          .Select(y => y.FullName)
                                          .ToList();
            Console.WriteLine("\nThe names of the artists, that have more than one album of PopRock genre :");
            nameArtistPopRock.ForEach(na => Console.Write("--{0}",na));



            // - print the name of the album that has highest Average duration of a song

            Album highestAverageDuration = Albums
                                                .Where(k => k.Songs.Select(h => h.Duration).Average() == Albums
                                                                               .Select(x => x.Songs.Select(y => y.Duration).Average())
                                                                               .Max())
                                                                               .SingleOrDefault();

            Console.WriteLine("\n\nThe name of the album that has highest Average duration of a song is : {0}", highestAverageDuration.Name);

            // - how many characters has the song that has the shortest Duration
            int countChSong = Songs
                                           .Where(s => s.Duration == Songs
                                                                           .Select(x => x.Duration)
                                                                           .Min())
                                           .SingleOrDefault().Name.Count();


            Console.WriteLine("\nThe song that has the shortest Duration has name of {0} characters.", countChSong);

            // - print the name and the genre of the album that has most songs
            Album albumMostSongs = Albums
                                        .Where(x => x.Songs.Count() == Albums
                                                                           .Select(sm => sm.Songs.Count())
                                                                           .Max())
                                                                           .SingleOrDefault();
            
            Console.WriteLine("\nThe name and the genre of the album that has most songs is : {0} - {1}", albumMostSongs.Name, albumMostSongs.Genre);

            // - print the name of the artist that has most songs
            Artist artistMostoSongs = Artists
                                          .Where(ar => ar.Albums.Select(p => p.Songs.Count()).Sum() == Artists
                                                                  .Select(x => x.Albums.Select(y => y.Songs.Count()).Sum())
                                                                  .Max())
                                                                  .SingleOrDefault();

            Console.WriteLine("\nThe name of the artist that has most songs is : {0}", artistMostoSongs.FullName);

            // - print the type of the artist(SoloArtist/Band) that has most albums published before year 2000
            Artist artistMostAlbums = Artists
                                            .Where(k => k.Albums.Count(y => y.Year < 2000) == Artists
                                                                    .Select(x => x.Albums.Count(y => y.Year < 2000))
                                                                    .Max())
                                                                    .SingleOrDefault();

            Console.WriteLine("\nThe artist(SoloArtist/Band) that has most albums published before year 2000 is : {0}", artistMostAlbums.ArtistType );

            // - print the average song duration, of the album that has most songs 
            double averageSongDuration = Albums
                                                .Where(alb => alb.Songs.Count() == Albums
                                                                            .Select(ms => ms.Songs.Count())
                                                                            .Max())
                                                                            .SingleOrDefault()
                                                                            .Songs
                                                                            .Select(c => c.Duration)
            ...
            Console.WriteLine("\nThe average song duration, of the album that has most songs is {0}.", averageSongDuration);


            // Bonus:
            // - print the longest song duration of the album that has least songs
            Song longestDurationLeastSong = Albums
                                        .Where(so => so.Songs.Count() == Albums
                                                                .Select(x => x.Songs.Count())
                                                                .Min())
                                                        .FirstOrDefault()
                                                        .Songs.Where(sd => sd.Duration == Albums
                                                          .Where(so => so.Songs.Count() == Albums
                                                               .Select(x => x.Songs.Count())
                                                                .Min())
                                                                     .FirstOrDefault().Songs.Select(s => s.Duration).Max())
                                                                     .SingleOrDefault();
            Console.WriteLine("\nThe longest song duration of the album that has least songs is : {0}", longestDurationLeastSong.Name);

            // - print the name of the album that has most songs that contain letter 'a' in the name
            Album albumNameContainsA = Albums
                                            .Where(al => al.Songs
                                            .Where(so => so.Name.ToLower().Contains('a')).Count() == Albums
                                            .Select(x => x.Songs
                                            .Where(y => y.Name.ToLower().Contains('a')).Count())
                                            .Max())
                                            .SingleOrDefault();
            Console.WriteLine("\nThe name of the album that has most songs that contain letter 'a' in the name is : {0}", albumNameContainsA.Name);

            // - print the name of the artist that has most songs that end with letter 'd'

        Artist artistSongsEndsD = Artists
                          .Where(ar => ar.Albums.Select(p => p.Songs.Where(n => n.Name.ToLower().EndsWith('d')).Count()).Sum() == Artists
                                          .Select(x => x.Albums.Select(y => y.Songs.Where(n => n.Name.ToLower().EndsWith('d')).Count()).Sum())
                                          .Max())
                                          .SingleOrDefault();
            Console.WriteLine("\nThe name of the artist that has most songs that end with letter 'd' is : {0}", artistSongsEndsD.FullName);
